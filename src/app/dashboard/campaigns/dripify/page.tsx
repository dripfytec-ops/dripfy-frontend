...
export default function DisparoDripifyPage() {
  ...
  const agendamentoValido = useMemo(() => {
    ...
    const minimo = new Date(Date.now() + 30 * 60_000);
    ...
    return (
      <div className="p-6 max-w-lg mx-auto">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 flex flex-col items-center text-center gap-3">
          ...
          <button onClick={() => router.push('/dashboard/campaigns/dripify')} className="btn-primary text-sm mt-2">
            Criar outra demanda
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 min-h-[360px]">
        {passo === 1 && (
          <div className="space-y-4">
            ...
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Número/canal de disparo (opcional)</label>
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea
                ...
                placeholder="Digite a mensagem... use *negrito*, _itálico_ e ~tachado~"
                rows={5}
            ...
            {mensagemTexto.trim() && (
              <div>
                <p className="text-xs text-gray-500 mb-1.5">Prévia</p>
                <div className="rounded-lg p-3" style={{ background: '#efeae2' }}>
                  <div className="max-w-[85%] rounded-2xl rounded-tr-sm px-3.5 py-2 shadow-sm ml-auto" style={{ background: '#d9fdd3' }}>
                    <p className="text-sm text-slate-900 whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: renderMarkdownWhats(mensagemTexto) }} />
                    {linkBotao && (
                      <div className="mt-2 pt-2 border-t border-black/10 text-center">
                        <span className="text-xs text-blue-600 font-medium">🔗 Abrir link</span>
                      </div>
                    )}
                  </div>
                </div>
        ...
        {passo === 5 && (
          <div className="space-y-4">
            ...
            {!agendamentoValido && (
              <p className="text-xs text-red-500 flex items-center gap-1"><AlertTriangle size={12} /> O agendamento precisa ser pelo menos 30 minutos no futuro.</p>
            )}
        ...
        {passo === 6 && (
          <div className="space-y-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between"><span className="text-gray-400">Canal</span><span className="text-gray-800">{canais.find((c) => c.id === canalId)?.nome || 'Sem preferência'}</span></div>
